...
impl<'a> BestAi<'a> {
    ...
    fn rensa_extend(&mut self, max_turn: usize, think_time_in_milli: u64) {
        ...
        let best = self.get_best(limit, &states);
    ...
    fn do_counter(&mut self) -> bool {
        ...
        if let Some(best_counter) = self.get_best(enemy_attack.1.obstacle * 3 / 2, &self_counter_states) {
    ...
    fn do_anti_counter(&mut self) -> bool {
        ...
        if let Some(enemy_counter_best) = self.get_best(200, &enemy_counter_states) {
    ...
    fn kill_bommer_emergency(&mut self) -> bool {
        if self.enemy.skill_guage >= 50 && self.cur_turn > 15 {
            self.maybe_bommer_emergency = true;
        }

        if self.enemy.skill_guage <= 20 {
            self.maybe_bommer_emergency = false;
        }

        if !self.maybe_bommer_emergency {
            return false;
        }
       ...
       let context = skill_plan::PlanContext {
            ...
            max_turn: 5,
            think_time_in_milli: 500,
            player: self.player.clone(),
            enemy_send_obstacles: &[],
            packs: &self.packs,
        };

        let replays = skill_plan::calc_skill_plan(&context, &mut self.rand, |result, decreased_skill_guage, _player, feature| {
            ...
            let feature_score =
                                ...
                                ;
            decreased_skill_guage as i64 * 1000000000 + obstacle_score as i64 * 1000000 + feature_score as i64
        });

        if let Some(r) = self.get_best_kill_bommer(&replays) {
            self.current_best = r;
            eprintln!("kill_bommer_emergency: {}", self.current_best.get_decreased_skill_guage());
        }
        true
    }
    ...
    fn get_best(&self, limit_obstacle: i32, states: &[replay::Replay]) -> Option<replay::Replay> {
        let mut max = -1;
        ...
        choosed.map(|s| s.clone())
    }

    fn get_best_kill_bommer(&self, states: &[replay::Replay]) -> Option<replay::Replay> {
        let mut max = -1;
        let mut choosed = None;
        states.iter().for_each(|s| {
            let val = s.get_decreased_skill_guage();
            if max < val {
                max = val;
                choosed = Some(s);
            }
