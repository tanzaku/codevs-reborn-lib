...
struct BeamState {
    ...
    score: i64,
    decreased_skill_guage: i32,
    actions: u128,
}

impl BeamState {
    fn new(player: player::Player, score: i64, decreased_skill_guage: i32, actions: u128) -> Self {
        Self { player, score, decreased_skill_guage, actions, }
    }
}
...
// 一手進める
fn do_action<F>(player: &mut player::Player, search_turn: usize, decreased_skill_guage: i32, context: &PlanContext, action: &action::Action, calc_score: &F) -> (i64, i64, i32)
    where F: Fn(&action::ActionResult, i32, &player::Player, &board::Feature) -> i64 + Sync + Send
{
    ...
    let decreased_skill_guage = decreased_skill_guage + result.skill_guage;
    let score = calc_score(&result, decreased_skill_guage, player, &feature);
    let eval_score = calc_score(&eval_result, decreased_skill_guage, player, &feature);
    (score, eval_score, decreased_skill_guage)
}

// ビームサーチ
pub fn calc_skill_plan<F>(context: &PlanContext, rand: &mut rand::XorShiftL, calc_score: F) -> Vec<replay::Replay>
    where F: Fn(&action::ActionResult, i32, &player::Player, &board::Feature) -> i64 + Sync + Send
{
        ...
        (0..context.max_turn).for_each(|search_turn| {
            ...
            if let Some(b) = heaps[search_turn].pop() {
                let results: Vec<_> = actions.par_iter().map(|a| {
                    ...
                    let (score, eval_score, decreased_skill_guage) = do_action(&mut player, search_turn, b.decreased_skill_guage, context, a, &calc_score);
                    let actions = push_action(b.actions, a);
                    
                    Some((player, score, eval_score, decreased_skill_guage, actions))
                }).collect();

                results.into_iter().filter(|x| x.is_some()).map(|x| x.unwrap()).for_each(|x| {
                    let (player, score, eval_score, decreased_skill_guage, actions) = x;
                    ...
                    if search_turn + 1 < context.max_turn {
                        heaps[search_turn + 1].push(BeamState::new(player.clone(), eval_score, decreased_skill_guage, actions));
                    }
